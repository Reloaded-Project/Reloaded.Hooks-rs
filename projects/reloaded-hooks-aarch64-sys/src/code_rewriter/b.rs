...
pub(crate) fn rewrite_b(
    ...
    if !(-0x02000000..=0x01FFFFFF).contains(&new_offset) {
        ...
        if !(-0x40000000..=0x3FFFFFFF).contains(&new_offset) {
            // MOV <immediate> + BR
            return rewrite_b_immediate(orig_target as usize, scratch_reg);
        }

        // ADRP + (ADD) + BR
        return rewrite_b_4gib(new_address, orig_target as usize, scratch_reg);
    }

    // Rewrite as regular branch.
    // - B
    let instruction = B::assemble_b(new_offset as i32);
...
pub(crate) fn rewrite_b_4gib(
    ...
    let branch = BranchRegister::new_br(scratch_register).0;
...
pub(crate) fn rewrite_b_immediate(
    ...
    let mov_instr = emit_mov_const_to_reg(scratch_register, orig_target as usize); // Assuming emit_mov_const_to_reg can handle this as well
    let b_instr = BranchRegister::new_br(scratch_register);
...
mod tests {
    ...
    // [Last Resort, Move] || b.eq #0 -> mov immediate + br x17 || (We add +4GiB to branch offset)
